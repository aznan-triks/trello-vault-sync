...
Cas "conflit réel" **reproduit et vérifié**, sans accès `computer-use`/Obsidian (refusé par l'utilisateur ce tour) : script de test jetable (`tests/_adhoc_conflict_probe.test.ts`, supprimé après usage) appelant le vrai code de production (`TrelloClient.getCard` + `decideForCard`, exactement ce que `resolveConflict` utilise) contre la vraie carte Trello de test (`6a98d25d488ecd2a8d62d3c5`) et la vraie note (`test-vault/Projects/Ideas/1.md`), les deux divergés en même temps (carte modifiée via API, note modifiée sur disque). Résultat : `decision.direction === "conflict"` confirmé, `npx vitest run` vert. Carte et note remises à leur état d'origine après coup (`afterAll`). Reste non couvert par ce probe : le rendu visuel du `ConflictModal` lui-même (nécessite Obsidian à l'écran) — logique inchangée par ce fix, risque résiduel jugé faible comme noté ci-dessus.
