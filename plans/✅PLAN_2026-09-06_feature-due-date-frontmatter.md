...
Les 4 cas de test critiques **rejoués contre la vraie carte de test** (`6a98d25fc8d7eb535bfce1b5`, notée non liée à un autre scénario), sans accès `computer-use`/Obsidian (refusé par l'utilisateur ce tour) : script de test jetable (`tests/_adhoc_due_probe.test.ts`, supprimé après usage) appelant le vrai code de production (`TrelloClient.updateCard`/`getCard` + `syncNoteWithCard`, exactement le chemin utilisé par `syncNote.ts`) avec un `FakeVault` (déjà utilisé par la suite officielle) en guise de note. Les 4 cas :
1. `card.due` fixé côté Trello → pull écrit `trello_due` avec la chaîne ISO exacte. ✅
2. `card.due` remis à `null` côté Trello → pull retire la clé `trello_due`. ✅
3. `trello_due` local édité et note plus récente que la carte → push écrit la nouvelle date sur la carte (`getCard` refait après coup le confirme). ✅
4. `trello_due` local malformé (`"not-a-date"`) et note plus récente → push relance l'appel API, Trello répond 400, `TrelloError` remonte sans écriture partielle. ✅
`npx vitest run` vert (4/4), carte de test remise à `due: null`/`desc: ""` après coup.
Reste non couvert par ce probe (nécessite Obsidian à l'écran) : l'écriture réelle via `app.fileManager.processFrontMatter` (le probe utilise le parseur YAML simplifié de `FakeVault`, pas `ObsidianVault.ts`) et la checklist visuelle §10. Risque résiduel jugé faible : `ObsidianVault.ts` réutilise la primitive `writeFrontmatter` déjà exercée par d'autres features (labels, ribbon) elles-mêmes vérifiées visuellement en v1.9.0-1.9.2.
